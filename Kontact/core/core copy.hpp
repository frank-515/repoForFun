...
public:
    ...
    JSON_BIND(Student, studentName, phoneNumber, className, studentNumber, tags);

    Student(studentName_t studentName_, phoneNumber_t phoneNumber_, className_t className_, std::string studentNumber) 
    : studentName(studentName_), phoneNumber(phoneNumber_), className(className_) , studentNumber(studentNumber) {}
    Student(studentName_t studentName_) : studentName(studentName_), phoneNumber("nil"), className("nil") , studentNumber("nil"){}
...
public:
    ...
    int find_snumber(const std::string &studentNumber){
        for (auto it = students.begin(); it != students.end(); it++){
            if (it->studentNumber == studentNumber) return it - students.begin();
        }
        return -1;
    }
    ...
    bool remove_snumber(const std::string &studentNumber){
        int index = find_snumber(studentNumber);
        if (index >= students.size() || index < 0) return false;
        students.erase(students.begin() + index);
        return true;
    }
    ...
    bool modify_snumber(int index, const std::string &studentNumber){
        if (index >= students.size() || index < 0) return false;
        students.at(index).studentNumber = studentNumber;
    }
